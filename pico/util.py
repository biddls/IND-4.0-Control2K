...
def round_table(table, decimals=5):
    rounded_table = []
    for row in table:
        rounded_row = [round(num, decimals) for num in row]
        rounded_table.append(rounded_row)
    return rounded_table


if __name__ == "__main__":
    file = b"../data/pre_processed2.csv".decode(encoding="utf-8")
    df = pd.read_csv(file, index_col='time')
    print(df.columns)
